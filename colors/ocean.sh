#!/bin/sh
#
# Reads a file, replaces Ocean color names enclosed in curly braces "{}" with their RGB values, outputs to stdout.
#

COLORS=$(dirname "$0")/ocean.txt

usage() {
	{
		echo "Usage: "$(basename "$0")" [options] filename"
		echo
		echo "Options:"
		echo -e "      --help                 print this help, then exit"
		echo -e "  -l, --light                switch colors to light color scheme (default is dark)"
		echo -e "  -H, --high-contrast        adjust color scheme to high-contrast"
		echo -e "  -q, --quote                enclose color values in double quotes (\")"
		echo -e "  -c, --c-prefix             use C-style hexadecimal prefix (0x) instead of hash (#)"
		echo -e "  -s, --script               instead of processing a file, output a sed script file to stdout"
	} >&2
	exit 2
}

quote=
prefix='#'
options=$*

while test $# -gt 0; do
	case $1 in
		--help)
			usage
		;;
		-l|--light)
			light=1
		;;
		-H|--high-contrast)
			high_contrast=1
		;;
		-q|--quote)
			quote='"'
		;;
		-d|--decimal)
			prefix=
			decimal=1
		;;
		-c|--c-prefix)
			prefix='0x'
		;;
		-s|--script)
			script=1
		;;
		-?*)
			echo "unrecognized option \"$1\"" >&2
			exit 1
		;;
		*)
			test "$1" = '-' && read_stdin=1 || filename="$1"
		;;
	esac

	shift
done

test "$read_stdin$filename$script" = "" && usage
test "$filename" && exec <"$filename"

declare -A colors
while read name tmp color; do
	test "$name" -a "${name:0:1}" != "#" -a "$tmp" = "=" -a "$color" || continue
	[[ $decimal ]] && let color=0x$color
	colors[$name]=$color
done <"$COLORS"

swap() {
	local tmp=${colors[$1]}
	colors[$1]=${colors[$2]}
	colors[$2]=$tmp
}

assign() {
	colors[$1]=${colors[$2]}
}

if test "$light"; then
	swap base00 base0
	swap base01 base1
	swap base02 base2
	swap base03 base3
fi

if test "$high_contrast"; then
	assign base01 base00
	assign base00 base0
	assign base0 base1
	assign base1 base2
	assign base2 base3
fi

if test "$script"; then
	echo '#!/bin/sed -f'
	echo '#'
	echo -n '# Ocean Colors ('
	test "$light" && echo -n "light" || echo -n "dark"
	test "$high_contrast" && echo -n ', high-contrast'
	echo -e ')\n#\n# This script was generated by `ocean.sh '$options'`.\n#\n'
	for name in ${!colors[*]}; do
		echo "s,{"$name"},$quote$prefix"${colors[$name]}"$quote,g"
	done | sort
else
	cmd='sed'
	for name in ${!colors[*]}; do
		cmd="$cmd -e s,{"$name"},$quote$prefix"${colors[$name]}"$quote,g"
	done
	$cmd
fi
